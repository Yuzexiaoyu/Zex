...
local user_opts = {
    ...
    tick_delay_follow_display_fps = true,  -- use display FPS as the minimum redraw interval
    ...
    volumebar_wheel_down_command = "no-osd add volume -5",
    volumebar_wheel_up_command = "no-osd add volume 5",
    ...
    file_loop_mbtn_left_command = "no-osd cycle-values loop-file inf no",
    file_loop_mbtn_right_command = "ignore",
...
local window_control_box_width = 120
...
local function set_osc_styles()
    ...
    osc_styles = {
        ...
        window_control = "{\\1c&H" .. osc_color_convert(user_opts.window_controls_color) .. "&\\fs21\\fn" .. icons.iconfont .. "}",
...
local state = {
    ...
    -- ZEX：帧内鼠标位置缓存。mouse_hit_coords 是所有命中判定的唯一入口，每次调用都
    -- 要跨 Lua→C 问一次 mp.get_mouse_pos()，一轮 render() 下来约 100 次；而一帧之内
    -- （300Hz 屏约 3ms）鼠标不会动，结果完全一样。tick 开头置 nil 失效，帧内首次求值后复用
    mouse_cache_x = nil, mouse_cache_y = nil,
    ...
    drawer_osd = mp.create_osd_overlay("ass-events"),  -- 抽屉独立 overlay：滚动/动画只重绘它，不拖累整条 OSC
    logo_osd = mp.create_osd_overlay("ass-events"),
    speed_overlay = mp.create_osd_overlay("ass-events"),
    ...
    -- ZEX：播放列表侧边抽屉
    playlist_drawer_visible = false,        -- 抽屉开/关
    playlist_drawer_scroll = 0,             -- 列表滚动偏移（首行 playlist 下标，0 基）
    playlist_drawer_loop_timer = nil,       -- 抽屉自驱动循环定时器（开/关动画期间运行）
    playlist_meta_poll = nil,               -- ZEX：抽屉开着时上次回读 queue.json 的时刻（0.5s 节流）
    playlist_drawer_loop_active = false,    -- 循环是否在跑（true 时 render 不再重复画抽屉）
    playlist_drawer_loop_fps = 60,          -- 当前循环速率（= display-fps，显示器刷新率）
    playlist_drawer_hover = nil,            -- 悬停的行（playlist 下标，0 基）
    playlist_drawer_close_hover = false,    -- 悬停在抽屉右上角 ×
    playlist_drawer_element = nil,          -- prepare_elements 后引用的抽屉元素
    playlist_drawer_sel = nil,              -- 键盘选中行（0 基），区别于鼠标悬停
    playlist_drawer_sel_season = nil,       -- 选中落在季标题行时存季号（此时 sel 为 nil）；nil=选中剧集行
    playlist_drawer_jump_hover = false,     -- 悬停在头部「定位到当前集」按钮
    playlist_drawer_anim_start = nil,       -- 滑入/滑出动画起始时刻（mp.get_time）
    playlist_drawer_anim_dir = nil,         -- "in" / "out"
    playlist_drawer_closing = false,        -- 滑出动画播完才真正隐藏
    playlist_drawer_pending_play = nil,     -- 收起动画播完后再切集的待播下标（避免加载抢占动画）
    playlist_drawer_drag = false,           -- 正在拖滚动条
    playlist_drawer_keys_on = false,        -- 抽屉键盘 binding section 是否已启用
    playlist_drawer_collapsed = {},         -- 收起的季 {[season_number]=true}
    playlist_drawer_hover_season = nil,     -- 悬停的季标题行（季号），区别于悬停剧集行
    playlist_drawer_cur_season = nil,       -- 当前播放集所在的季（跨季自动收起/展开用）
    -- ZEX：倍速档位悬停面板
    speed_panel_open = false,           -- 面板是否显示
    speed_panel_closing = false,        -- 收起动画播放中（播完才 open=false）
    speed_panel_hover = nil,            -- 悬停档位索引（1-6）
    speed_panel_timer = nil,            -- 延迟关闭 timer
    speed_panel_anim_start = nil,       -- 弹出/收起动画起始时刻（mp.get_time）
    speed_panel_anim_dir = nil,         -- "in" / "out"
    speed_panel_suppress = false,       -- 手动收起后抑制悬停自动重开，光标离开按钮/面板才解除
    speed_element = nil,                -- osc_init 缓存的 speed 按钮引用（面板靠它的 hitbox 定位）
    -- 选档后 guard 缓冲窗口：面板收起动画播完、open=false 后，捕获区恢复到底部栏，
    -- 但鼠标还停在原档位位置（现在属于视频空白）。此时双击档位 / 快速切换档位的
    -- 第二次点击会漏成 MBTN_LEFT cycle pause（播放/暂停）。guard 期间保持全屏捕获 +
    -- 把第二次点击当成面板逻辑处理，到期后恢复
    speed_panel_guard = nil,            -- 未过期时刻（mp.get_time），nil=不生效
    speed_panel_guard_timer = nil,      -- 到期自动恢复的 timer
    -- ZEX：进度条预览图 hover 延迟 —— 鼠标悬停稳定一段时间后才请求缩略图，
    -- 移开即取消；拖拽（force_seek_tooltip）不受延迟影响，立即显示
    seek_thumb_ready = false,           -- 延迟到期置 true，此时才允许绘制预览图
    seek_thumb_timer = nil,             -- 延迟 timer（到期置 ready + request_init）
}

state.speed_overlay.z = 2100 -- 盖在主 OSD（1000）与音量提示（2000）之上
...
-- ZEX：结果按帧缓存（state.mouse_cache_x/y，tick 开头失效）。这是 mouse_hit_coords →
-- mouse_hit / mouse_in_area 的底座，一轮 render() 要走约 100 次，每次都跨 Lua→C 调
-- mp.get_mouse_pos()。帧内鼠标不会动，首次求值后直接复用。
-- 三条分支的结果都进缓存（含触摸态和窗口外的 -1,-1），否则那两条路径拿不到缓存收益
local function get_virt_mouse_pos()
    if state.mouse_cache_x ~= nil then
        return state.mouse_cache_x, state.mouse_cache_y
    end
    local rx, ry
    if recently_touched() then
        local sx, sy = get_virt_scale_factor()
        rx, ry = state.last_touchX * sx, state.last_touchY * sy
    ...
    state.mouse_cache_x, state.mouse_cache_y = rx, ry
    return rx, ry
end
...
-- ZEX：前向声明。倍速面板的 speed_panel_mouse_over 定义在文件后部（面板实现区，
-- 依赖 state 初始化后的常量/函数），但 render_elements 的 seekbar 分支需要在这里
-- 提前调用它来屏蔽面板遮挡区域的进度条悬停（缩略图/tooltip 会透过面板冒出来）。
local speed_panel_mouse_over
...
local function prepare_elements()
    ...
    for _,element in pairs(elements) do
        ...
        if element.type == "box" and element.name ~= "playlist_drawer" then
            --draw box
...
-- 前向声明：draw_playlist_drawer 定义在 render_elements 之后（见抽屉段落），
-- 这里先声明本地名，render_element 闭包才能捕获到它
local draw_playlist_drawer
...
local function render_elements(master_ass, osc_vis, wc_vis)
    local function render_element(n)
        ...
        local is_drawer = element.layout.group == "drawer"
        if (is_top and not wc_vis) or (is_drawer and not state.playlist_drawer_visible)
            or (not is_top and not is_drawer and not osc_vis) then return end
        ...
        if element.type == "slider" then
            if element.name ~= "persistent_seekbar" then
                ...
                if element.slider and element.slider.tooltipF ~= nil and element.enabled then
                    ...
                    -- ZEX：倍速面板弹出时，鼠标在面板/按钮热区内 → 屏蔽进度条 hover。
                    -- 面板是独立 overlay，不参与元素 hitbox，鼠标在面板上时这里
                    -- 的 mouse_hit(seekbar) 依然命中 → 缩略图/tooltip 会透过面板冒出
                    -- ZEX：预览图 hover 延迟 —— 悬停稳定 0.4s 后才显示缩略图，
                    -- 期间只启动 timer；鼠标移开由下方 elseif 分支取消。
                    -- 拖拽（force_seek_tooltip）是明确交互，不受延迟限制，立即显示
                    local seek_hover = mouse_hit(element) and not (state.speed_panel_open and speed_panel_mouse_over())
                    if seek_hover and not force_seek_tooltip and element.thumbnailable
                        and not state.seek_thumb_ready and state.seek_thumb_timer == nil then
                        state.seek_thumb_timer = mp.add_timeout(0.4, function()
                            state.seek_thumb_ready = true
                            state.seek_thumb_timer = nil
                            request_init()
                        end)
                    end
                    if seek_hover or force_seek_tooltip then
                        ...
                        -- Thumbfast logic
                        if element.thumbnailable and not thumbfast.disabled and osd_w
                            and (force_seek_tooltip or state.seek_thumb_ready) then
                    ...
                    elseif element.thumbnailable and thumbfast.available then
                        -- ZEX：鼠标离开进度条 → 取消待触发的延迟并清掉预览图
                        if state.seek_thumb_timer then
                            state.seek_thumb_timer:kill()
                            state.seek_thumb_timer = nil
                        end
                        state.seek_thumb_ready = false
        ...
        elseif element.type == "button" then
            ...
            if element.tooltipF ~= nil and element.enabled and not seeking_with_force_tooltip and user_opts.tooltip_hints
                and not (element.name == "speed" and state.speed_panel_open) then
                local hb = element.hover_box
        ...
        elseif element.name == "playlist_drawer" then
            -- 抽屉改画到独立 drawer_osd overlay（见 playlist_drawer_draw_overlay），
            -- 滚动/动画逐帧只重绘抽屉，不再连带整条 OSC 一起重绘。
            -- 元素本身仍留在列表里：命中判定、输入、拖拽事件照常工作。
        end
...
local function window_controls()
    ...
    local first_geo  = {x = controlbox_left + 20,  y = button_y, an = 5, w = 40, h = wc_geo.h}
    local second_geo = {x = controlbox_left + 60, y = button_y, an = 5, w = 40, h = wc_geo.h}
    local third_geo  = {x = controlbox_left + 100, y = button_y, an = 5, w = 40, h = wc_geo.h}
    ...
    if user_opts.window_controls then
        ...
        -- ZEX：右上角去掉了最大化/还原按钮（不再有控制全屏状态的按钮），
        -- 最小化移到靠右，与关闭紧凑排列
        wc_button("minimize", second_geo, user_opts.windowcontrols_min_hover) -- Minimize: 🗕
    end
    ...
    if user_opts.show_window_title then
        ...
        lo.geometry = {x = math.max(20, titlebox_left + 4), y = button_y + math.floor(user_opts.window_title_font_size / 2), an = 1, w = osc_param.playresx - 50, h = wc_geo.h}
        lo.group = "top"
        lo.alpha[3] = 0
...
local function setup_bg_elements(posX, posY, osc_w, osc_h, osc_alpha3, wc_alpha3)
    -- ZEX：去掉顶栏/底栏的黑色透明遮罩（osc_fade_bg / window_bar_alpha_bg）。
    -- 原实现用 \1c&H0& 黑主色填充 + \bord100\blur100 黑描边糊开成渐变晕；
    -- 只改配置（osc_fade_strength=0）去不掉主色填充，故直接不创建
    return
end
...
layouts["default"] = function ()
    ...
    right_side_button("playlist", 550, playlist_button)
...
--
-- 播放列表侧边抽屉（ZEX）
-- 点击底部栏播放列表按钮 = 右侧滑出抽屉，覆盖在视频右侧。
-- 点击行换集（playlist-play-index）、滚轮/拖滚动条滚动、× 或 Esc 或点抽屉外关闭；
-- ↑/↓ 选行、Enter 播放；点击季标题或 ←/→ 收起/展开该季。
-- 当前集高亮，已看集显示 ✓ 徽章 + 满格进度条，看到一半的显示部分进度条。
-- 复用 ModernZ 元素体系：抽屉是一个 layer 30 的 "drawer" 组元素，
-- 开关时切换 visible + request_init（prepare_elements 会移除不可见元素）。
--
local playlist_drawer_w = 360           -- 抽屉宽度（virt px）
local playlist_drawer_row_h = 52        -- 行高（两行文字 + 进度条）
local playlist_drawer_row_gap = 4       -- 行间距
local playlist_drawer_pad = 12          -- 内边距
local playlist_drawer_header_h = 44     -- 头部高度
local playlist_drawer_season_h = 30     -- 季标题行高（比剧集行矮，层级分明）
local playlist_drawer_fs = 15           -- 正文（单集标题）字号
local playlist_drawer_sub_fs = 12       -- 次要信息（时长/已看）字号
local playlist_drawer_header_fs = 16    -- 头部字号
local playlist_drawer_anim_dur = 0.18   -- 滑入/滑出时长（秒）

-- 抽屉矩形（virt 坐标），由元素 hitbox 得出。
-- 第二个返回组带上滑入动画的 X 偏移 —— 命中判定和绘制必须用同一套坐标，
-- 否则动画期间点击会落在终点位置
local function playlist_drawer_anim_offset()
    if not state.playlist_drawer_anim_start then return 0 end
    local t = (mp.get_time() - state.playlist_drawer_anim_start) / playlist_drawer_anim_dur
    if t >= 1 then
        -- 动画结束：滑出的话此时才真正隐藏元素
        state.playlist_drawer_anim_start = nil
        if state.playlist_drawer_anim_dir == "out" then
            state.playlist_drawer_anim_dir = nil
            state.playlist_drawer_closing = false
            state.playlist_drawer_visible = false
            render_wipe(state.drawer_osd)   -- 清掉独立 overlay，避免残留画面
            -- 收起动画播完：此刻抽屉已隐藏，再执行延迟的切集，加载不再抢占动画帧
            if state.playlist_drawer_pending_play ~= nil then
                local i = state.playlist_drawer_pending_play
                state.playlist_drawer_pending_play = nil
        ...
        return state.playlist_drawer_visible and 0 or playlist_drawer_w
    end
    -- ease-out cubic：起步快、收尾稳，和 UI 里常见的抽屉手感一致
    local eased = 1 - (1 - t) ^ 3
    local progress = state.playlist_drawer_anim_dir == "out" and eased or (1 - eased)
    playlist_drawer_loop_start()   -- 抽屉自驱动循环：动画期间逐帧只重绘抽屉 overlay
    return playlist_drawer_w * progress
end

local function playlist_drawer_rect()
    local e = state.playlist_drawer_element
    if not e or not e.hitbox then return nil end
    return e.hitbox.x1, e.hitbox.y1, e.hitbox.x2, e.hitbox.y2
end

-- 光标是否在抽屉上（抽屉关着时恒 false）。
-- 给 process_event 判断要不要跳过底部栏的 show/hide 逻辑用。
-- 动画期间面板在移动，必须按加了偏移的实际位置算
local function playlist_drawer_mouse_over()
    if not state.playlist_drawer_visible then return false end
    local x1, y1, x2, y2 = playlist_drawer_rect()
    if not x1 then return false end
    local dx = playlist_drawer_anim_offset()
    return mouse_hit_coords(x1 + dx, y1, x2 + dx, y2)
end

-- ZEX 写的播放列表边车（queue.json）：m3u 的 EXTINF 只装得下一个标题字符串，
-- 集号/已看/续播位置/时长都从这里来。数组下标与 playlist 下标一一对应，
-- count 对不上就整个丢弃退回纯标题模式（列表被 loadlist 换过但边车还没重写）
local playlist_meta = {items = nil, mtime = nil}

local function playlist_drawer_load_meta()
    local path = mp.command_native({"expand-path", "~~/queue.json"})
    local info = utils.file_info(path)
    if not info or not info.is_file then
        playlist_meta.items, playlist_meta.mtime = nil, nil
        return
    end
    if playlist_meta.items and playlist_meta.mtime == info.mtime then return end
    local f = io.open(path, "r")
    if not f then
        playlist_meta.items = nil
        return
    end
    local text = f:read("*all")
    f:close()
    local doc = utils.parse_json(text or "")
    playlist_meta.mtime = info.mtime
    playlist_meta.items = (type(doc) == "table" and type(doc.items) == "table") and doc.items or nil
end

-- 播放列表行缓存：内容只随 playlist/当前集/边车 mtime 变化，
-- 每帧全量 get_property_native("playlist") 太浪费
local playlist_drawer_cache = {count = -1, pos = -1, mtime = -1, rows = {}}

-- 播放列表行。meta 命中时带 season/episode/watched/watched_ms/runtime，
-- 没命中就只有 m3u 标题（title = "剧名 S01E01 单集名"）
local function playlist_drawer_rows()
    local pos = mp.get_property_number("playlist-pos", -1)
    local mtime = playlist_meta.mtime or -1
    if playlist_drawer_cache.count == state.playlist_count
        and playlist_drawer_cache.pos == pos
        and playlist_drawer_cache.mtime == mtime then
        return playlist_drawer_cache.rows
    end
    local rows = {}
    local pl = mp.get_property_native("playlist") or {}
    -- 边车条目数必须和 playlist 完全一致，否则下标对不上，宁可不用
    local meta = playlist_meta.items
    if meta and #meta ~= #pl then meta = nil end
    for i, entry in ipairs(pl) do
        local t = entry.title
        if not t or t == "" then t = entry.filename end
        ...
        if m then
            -- 单集标题优先用边车里的（不含剧名/季集前缀，行里更好读）
            if type(m.t) == "string" and m.t ~= "" then row.title = m.t end
            row.season = m.s
            row.episode = m.e
            row.watched = m.w == true
            row.watched_ms = tonumber(m.ms) or 0
            row.runtime = tonumber(m.rt) or 0
        end
        rows[#rows + 1] = row
    end
    playlist_drawer_cache.count = state.playlist_count
    playlist_drawer_cache.pos = pos
    playlist_drawer_cache.mtime = mtime
    playlist_drawer_cache.rows = rows
    return rows
end

-- ── 显示列表：季标题行 + 剧集行 ──
-- 收起的季只留标题行；没有边车数据（row.season 为空）时不插标题行，退回纯剧集列表
local function playlist_drawer_entry_h(entry)
    return entry.type == "season" and playlist_drawer_season_h or playlist_drawer_row_h
end

local function playlist_drawer_display()
    local rows = playlist_drawer_rows()
    local entries = {}
    local cur_season = nil
    local cur_header = nil
    for _, row in ipairs(rows) do
        if row.season and row.season ~= cur_season then
            cur_season = row.season
            cur_header = {type = "season", season = cur_season, count = 0, watched = 0}
            entries[#entries + 1] = cur_header
        end
        -- 季标题行的统计不受收起状态影响：收起了也要报「x/y 已看」
        if cur_header then
            cur_header.count = cur_header.count + 1
            if row.watched then cur_header.watched = cur_header.watched + 1 end
        end
        if not (cur_season and state.playlist_drawer_collapsed[cur_season]) then
            entries[#entries + 1] = {type = "ep", row = row}
        end
    end
    return entries
end

-- 找某个 playlist 下标（0 基）在显示列表里的下标（0 基）。所在季被收起时找不到
local function playlist_drawer_find_entry(pl_idx)
    for i, entry in ipairs(playlist_drawer_display()) do
        if entry.type == "ep" and entry.row.idx == pl_idx then return i - 1 end
    end
    return nil
end

-- 从 scroll（0 基显示下标）开始能完整放下的条目数。行高不一（季标题矮），只能实量
local function playlist_drawer_fit_count(scroll)
    local x1, y1, x2, y2 = playlist_drawer_rect()
    if not x1 then return 1 end
    local list_h = (y2 - y1) - playlist_drawer_header_h - playlist_drawer_pad * 2
    local entries = playlist_drawer_display()
    local h, n = 0, 0
    for i = scroll + 1, #entries do
        local eh = playlist_drawer_entry_h(entries[i]) + playlist_drawer_row_gap
        if h + eh > list_h then break end
        h = h + eh
        n = n + 1
    end
    return math.max(1, n)
...
-- 最大滚动偏移：最后一页正好填满列表时的起始下标（0 基）
local function playlist_drawer_max_scroll()
    local x1, y1, x2, y2 = playlist_drawer_rect()
    ...
    for i = #entries, 1, -1 do
        h = h + playlist_drawer_entry_h(entries[i]) + playlist_drawer_row_gap
        -- i 是 1 基：条目 i+1..N 放满一页时，0 基起点正好是 i
        if h > list_h then return math.min(i, math.max(0, #entries - 1)) end
    end
    return 0
end

-- 滚动（delta 为显示条目数，正向下）。整数跳变（滚轮/翻页）
local function playlist_drawer_scroll_by(delta)
    state.playlist_drawer_scroll = limit_range(0, playlist_drawer_max_scroll(),
        state.playlist_drawer_scroll + delta)
    request_tick()
end

-- 把某个显示条目（0 基）滚进可视区（键盘选行、定位当前集都用它）
local function playlist_drawer_reveal(i)
    if i < state.playlist_drawer_scroll then
        state.playlist_drawer_scroll = i
    else
        -- 从当前起点量不下就逐条往后挪，fit_count 至少为 1，循环必然收敛
        while state.playlist_drawer_scroll < i
            and i >= state.playlist_drawer_scroll + playlist_drawer_fit_count(state.playlist_drawer_scroll) do
            state.playlist_drawer_scroll = state.playlist_drawer_scroll + 1
        end
    end
    state.playlist_drawer_scroll = limit_range(0, playlist_drawer_max_scroll(), state.playlist_drawer_scroll)
end

-- 滚到当前集并选中它（所在季被收起时先展开，否则永远定位不到）
local function playlist_drawer_goto_current()
    local pos = mp.get_property_number("playlist-pos", -1)
    if pos < 0 then return end
    local row = playlist_drawer_rows()[pos + 1]
    if row and row.season then state.playlist_drawer_collapsed[row.season] = nil end
    state.playlist_drawer_sel = pos
    state.playlist_drawer_sel_season = nil
    local di = playlist_drawer_find_entry(pos)
    if di then playlist_drawer_reveal(di) end
    request_tick()
end

-- 收起/展开某季。收起后滚动位置可能被顶出界，钳回合法范围
local function playlist_drawer_toggle_season(season)
    if state.playlist_drawer_collapsed[season] then
        state.playlist_drawer_collapsed[season] = nil
    else
        state.playlist_drawer_collapsed[season] = true
    end
    state.playlist_drawer_scroll = limit_range(0, playlist_drawer_max_scroll(), state.playlist_drawer_scroll)
    request_tick()
end
...
-- 键盘 ←/→：收起/展开选中行所在的季
local function playlist_drawer_sel_set_collapse(collapsed)
    local sel = state.playlist_drawer_sel
    if sel == nil then return end
    local row = playlist_drawer_rows()[sel + 1]
    if not (row and row.season) then return end
    state.playlist_drawer_collapsed[row.season] = collapsed and true or nil
    if not collapsed then
        -- 展开后选中行重新可见，把它滚回来
        local di = playlist_drawer_find_entry(sel)
        if di then playlist_drawer_reveal(di) end
    end
    state.playlist_drawer_scroll = limit_range(0, playlist_drawer_max_scroll(), state.playlist_drawer_scroll)
    request_tick()
...
-- 前向声明：键盘 binding 的开关要在 toggle 里调，但定义依赖 toggle 本身
local playlist_drawer_set_keys

local function playlist_drawer_close()
    if not state.playlist_drawer_visible or state.playlist_drawer_closing then return end
    -- 播滑出动画，动画结束时 playlist_drawer_anim_offset 才真正把 visible 置 false
    state.playlist_drawer_closing = true
    state.playlist_drawer_anim_dir = "out"
    state.playlist_drawer_anim_start = mp.get_time()
    state.playlist_drawer_drag = false
    playlist_drawer_set_keys(false)
    playlist_drawer_loop_start()   -- 滑出动画改由抽屉独立循环驱动
end

-- 默认收起状态：只展开正在播放的季，其它季全部收起。每次打开抽屉时重置
local function playlist_drawer_collapse_to_current()
    local pos = mp.get_property_number("playlist-pos", -1)
    local rows = playlist_drawer_rows()
    local cur = (pos >= 0 and rows[pos + 1]) and rows[pos + 1].season or nil
    local collapsed = {}
    for _, row in ipairs(rows) do
        if row.season and row.season ~= cur then collapsed[row.season] = true end
    end
    state.playlist_drawer_collapsed = collapsed
    state.playlist_drawer_cur_season = cur
end

-- 播放跨季时跟着走：旧季收起、新季展开（用户手动展开过的其它季不受影响）。
-- 抽屉开着时每帧在 draw 里调用
local function playlist_drawer_follow_season(rows)
    local pos = mp.get_property_number("playlist-pos", -1)
    local cur = (pos >= 0 and rows[pos + 1]) and rows[pos + 1].season or nil
    if cur == state.playlist_drawer_cur_season then return end
    if state.playlist_drawer_cur_season ~= nil then
        state.playlist_drawer_collapsed[state.playlist_drawer_cur_season] = true
    end
    if cur ~= nil then
        state.playlist_drawer_collapsed[cur] = nil
    end
    state.playlist_drawer_cur_season = cur
end

-- 开/关抽屉
local function playlist_drawer_toggle()
    if state.playlist_drawer_visible and not state.playlist_drawer_closing then
        playlist_drawer_close()
        return
    end
    playlist_drawer_load_meta()
    playlist_drawer_collapse_to_current()
    state.playlist_drawer_visible = true
    state.playlist_drawer_closing = false
    state.playlist_drawer_anim_dir = "in"
    state.playlist_drawer_anim_start = mp.get_time()
    state.playlist_drawer_pending_play = nil   -- 重新打开抽屉 = 取消延迟切集
    -- 打开时滚到当前集附近并预选中它
    playlist_drawer_goto_current()
    playlist_drawer_set_keys(true)
    request_init()
    playlist_drawer_loop_start()   -- 滑入动画 + 首次绘制由抽屉循环驱动
end

mp.add_key_binding(nil, "playlist-drawer-toggle", playlist_drawer_toggle)

-- 播放选中行
local function playlist_drawer_play_sel()
    local i = state.playlist_drawer_sel
    if i == nil then return end
    if i >= 0 and i < #playlist_drawer_rows() then
        -- 先收起抽屉再切集：切集瞬间 mpv 关旧开新、VO 短暂无输出，会抢占动画帧；
        -- 动画播完（anim_offset 的 out 收尾）后才真正 playlist-play-index
        state.playlist_drawer_pending_play = i
        playlist_drawer_close()
    end
end

-- 确认键（手柄 A / 键盘 Enter）：选中季标题行 → 展开/收起该季；选中剧集行 → 播放
local function playlist_drawer_activate()
    if state.playlist_drawer_sel_season ~= nil then
        playlist_drawer_toggle_season(state.playlist_drawer_sel_season)
        return
    end
    playlist_drawer_play_sel()
end

-- 滚动条拖拽开始：鼠标按住滚动条（thumb 或轨道任意处）时进入拖拽，
-- active_element 期间的 render 事件会每帧按鼠标位置更新滚动
local function playlist_drawer_start_drag()
    local x1, y1, x2, y2 = playlist_drawer_rect()
    if not x1 then return end
    local mx, my = get_virt_mouse_pos()
    if mx < 0 or my < 0 then return end
    -- 列表放得下就不存在滚动条，别拦这一下的点击
    if playlist_drawer_max_scroll() == 0 then return end
    -- 滚动条在右侧内边距处，宽 3 + 余量
    local bar_x1 = x2 - playlist_drawer_pad
    local list_y1 = y1 + playlist_drawer_pad + playlist_drawer_header_h
    local list_y2 = y2 - playlist_drawer_pad
    if mx >= bar_x1 - 4 and mx <= x2 and my >= list_y1 and my <= list_y2 then
        state.playlist_drawer_drag = true
        request_tick()
    end
end

-- 拖拽中每帧调用（draw_playlist_drawer 里也按鼠标位置算，这里保证点下就生效一次）
local function playlist_drawer_drag_tick()
    if not state.playlist_drawer_drag then return end
    local x1, y1, x2, y2 = playlist_drawer_rect()
    if not x1 then return end
    local mx, my = get_virt_mouse_pos()
    if mx < 0 or my < 0 then return end
    local list_y1 = y1 + playlist_drawer_pad + playlist_drawer_header_h
    local list_y2 = y2 - playlist_drawer_pad
    local max_scroll = playlist_drawer_max_scroll()
    if max_scroll == 0 then
        state.playlist_drawer_drag = false
        return
    end
    local frac = limit_range(0, 1, (my - list_y1) / math.max(1, list_y2 - list_y1))
    state.playlist_drawer_scroll = math.floor(frac * max_scroll + 0.5)
    playlist_drawer_draw_overlay()   -- 拖拽逐帧只重绘抽屉 overlay
end

-- 上下移动选中行：季标题行和剧集行都能聚焦（季标题上按确认展开/收起）。
-- 选中行所在季被收起时它不在显示列表里，从它后面的第一个可见行继续
local function playlist_drawer_move_sel(delta)
    local focus = {}
    for di, entry in ipairs(playlist_drawer_display()) do
        if entry.type == "season" then
            focus[#focus + 1] = {di = di - 1, kind = "season", season = entry.season}
        else
            focus[#focus + 1] = {di = di - 1, kind = "ep", pl = entry.row.idx}
        end
    end
    if #focus == 0 then return end
    local k = nil
    if state.playlist_drawer_sel_season ~= nil then
        for j, f in ipairs(focus) do
            if f.kind == "season" and f.season == state.playlist_drawer_sel_season then k = j break end
        end
    else
        local cur = state.playlist_drawer_sel
        if cur == nil then cur = mp.get_property_number("playlist-pos", 0) end
        for j, f in ipairs(focus) do
            if f.kind == "ep" and f.pl == cur then k = j break end
        end
        if not k then
            -- cur 不可见（所在季被收起）：落在它后面的第一个可见行
            k = #focus
            for j, f in ipairs(focus) do
                if f.kind == "ep" and f.pl > cur then k = j break end
            end
    ...
    k = limit_range(1, #focus, k + delta)
    local target = focus[k]
    if target.kind == "season" then
        state.playlist_drawer_sel_season = target.season
        state.playlist_drawer_sel = nil
    else
        state.playlist_drawer_sel = target.pl
        state.playlist_drawer_sel_season = nil
    end
    playlist_drawer_reveal(target.di)
    request_tick()
end

-- 抽屉专属键盘 section：只在抽屉开着时启用，关掉就把按键还给 mpv 默认绑定
-- （否则 Esc/方向键会被永久劫持）。用 forced section，和上面的 input 系列同构。
-- 这些绑定直接调用处理函数，不走 process_event —— 它们不属于任何鼠标元素
mp.set_key_bindings({
    {"UP",     function() playlist_drawer_move_sel(-1) end},
    {"DOWN",   function() playlist_drawer_move_sel(1) end},
    {"PGUP",   function() playlist_drawer_move_sel(-playlist_drawer_fit_count(state.playlist_drawer_scroll)) end},
    {"PGDWN",  function() playlist_drawer_move_sel(playlist_drawer_fit_count(state.playlist_drawer_scroll)) end},
    {"HOME",   function() playlist_drawer_move_sel(-1e9) end},
    {"END",    function() playlist_drawer_move_sel(1e9) end},
    {"LEFT",   function() playlist_drawer_sel_set_collapse(true) end},
    {"RIGHT",  function() playlist_drawer_sel_set_collapse(false) end},
    {"ENTER",  playlist_drawer_activate},
    {"ESC",    function() playlist_drawer_close() end},
    -- 手柄：抽屉开着时按键在抽屉内导航，不控制视频（forced section 优先于 input.conf 的默认绑定）
    -- A=季上展开/收起、剧集上播放，B/X=关闭，Y=定位当前集
    -- D-pad ↑/↓ 和右摇杆都不在这里：set_key_bindings 是普通绑定，长按的 repeat 事件被忽略
    -- （不会连续移动），所以它们改用 complex 绑定 + 自管定时器实现长按连续，见 playlist_drawer_set_keys
    {"GAMEPAD_ACTION_DOWN",     playlist_drawer_activate},
    {"GAMEPAD_ACTION_RIGHT",    function() playlist_drawer_close() end},
    {"GAMEPAD_ACTION_LEFT",     function() playlist_drawer_close() end},
    {"GAMEPAD_ACTION_UP",       function() playlist_drawer_goto_current() end},
}, "playlist_drawer_keys", "force")

-- 右摇杆滚动已移除（2026-08-07）：mpv OSD 的 libass 渲染对「列表内容连续变动」
-- 这种负载撑不起顺滑帧率，连续滚动在 mpv 内无解，导航回归 D-pad/滚轮离散模型。
-- 历史背景（保留）：mpv 的 --input-gamepad 是单键模型，右摇杆 X 轴微偏会发 neutral
-- UP 冲掉 Y 轴按住态，所以此前由 Rust 侧 gilrs 读轴值经 IPC 驱动——现已整体删除。

-- 抽屉内 D-pad ↑/↓ 长按连续移动选中行：complex 绑定拿 down/up 事件。
-- 按下立即动一格；按住 0.4s 后进入 0.05s/次的快速连发（主机风格首延迟）。
-- 注意 local 上限：状态用 state 字段、函数用全局名，禁止新增顶层 local
playlist_drawer_dpad_set = function(delta)
    if state.playlist_drawer_dpad_delta == delta then return end
    state.playlist_drawer_dpad_delta = delta
    if state.playlist_drawer_dpad_timer then
        state.playlist_drawer_dpad_timer:kill()
        state.playlist_drawer_dpad_timer = nil
    end
    if state.playlist_drawer_dpad_repeat then
        state.playlist_drawer_dpad_repeat:kill()
        state.playlist_drawer_dpad_repeat = nil
    end
    if delta ~= 0 then
        playlist_drawer_move_sel(delta)  -- 按下立即动一格
        state.playlist_drawer_dpad_repeat = mp.add_timeout(0.4, function()
            state.playlist_drawer_dpad_repeat = nil
            state.playlist_drawer_dpad_timer = mp.add_periodic_timer(0.05, function()
                playlist_drawer_move_sel(delta)
            end)
        end)
    end
end
playlist_drawer_dpad_event = function(delta)
    return function(e)
        if e.event == "down" then
            playlist_drawer_dpad_set(delta)
        elseif e.event == "up" then
            playlist_drawer_dpad_set(0)
        end
    end
end

-- ── 抽屉独立渲染循环（只驱动开/关滑入滑出动画）──
-- 动画不再走 OSC 的共享 request_tick（那会在每帧重绘整条底部栏 + 进度条 +
-- 抽屉），而是驱动一个独立的 ~60Hz 周期循环，只更新抽屉自己的 drawer_osd
-- overlay。循环只在动画期间运行，播完即停，不空烧 CPU。
-- 注意 local 上限：状态一律 state 字段、函数用全局名，禁止新增顶层 local
playlist_drawer_loop_needed = function()
    return state.playlist_drawer_visible and state.playlist_drawer_anim_start ~= nil
end

-- 动画渲染速率 = 显示器刷新率。mpv 是 display-resample 出帧，硬编码 1/60 会让
-- 高刷显示器上的动画每帧变化只有 60 次/秒，看着就掉帧。取不到刷新率才兜底 60。
playlist_drawer_loop_fps = function()
    local fps = effective_display_fps()
    if not fps then fps = 60 end
    return fps
end

playlist_drawer_loop_timer_start = function()
    local fps = playlist_drawer_loop_fps()
    state.playlist_drawer_loop_fps = fps
    state.playlist_drawer_loop_timer = mp.add_periodic_timer(1 / fps, playlist_drawer_loop_tick)
end

playlist_drawer_draw_overlay = function()
    if not state.playlist_drawer_visible then
        render_wipe(state.drawer_osd)
        return
    end
    local ass = assdraw.ass_new()
    draw_playlist_drawer(state.playlist_drawer_element, ass)
    if not state.playlist_drawer_visible then
        -- 滑出动画在 draw 中途结束：anim_offset 的 out 收尾已把 visible 置 false
        -- 并 render_wipe，同时返回 dx=360 让面板整体离屏。但面板描边是 \bord1
        -- 白线（ASS 边框沿路径两侧各扩 1px），离屏面板左缘正好贴到屏幕右缘
        -- x=playresx，会向可视区渗 1px。若这里再 set_osd，会把这 1px 白线
        -- 重新提交并永久钉在屏幕最右侧（wipe 已把 data 置空，dedup 判定为变化）。
        -- 所以动画结束那一帧只保持已清空状态，跳过提交。
        render_wipe(state.drawer_osd)
        return
    end
    set_osd(state.drawer_osd, osc_param.playresy * osc_param.display_aspect,
        osc_param.playresy, ass.text, 2000)
end

playlist_drawer_loop_tick = function()
    playlist_drawer_draw_overlay()
    if not playlist_drawer_loop_needed() then
        playlist_drawer_loop_stop()
    end
end

playlist_drawer_loop_start = function()
    if state.playlist_drawer_loop_active then return end
    if not playlist_drawer_loop_needed() then return end
    state.playlist_drawer_loop_active = true
    playlist_drawer_loop_timer_start()
end

playlist_drawer_loop_stop = function()
    state.playlist_drawer_loop_active = false
    if state.playlist_drawer_loop_timer then
        state.playlist_drawer_loop_timer:kill()
        state.playlist_drawer_loop_timer = nil
    end
end

-- 显示器刷新率变化（跨屏/动态刷新切换）时，动画循环按新速率重启
observe_cached("display-fps", function()
    if not state.playlist_drawer_loop_active then return end
    local fps = playlist_drawer_loop_fps()
    if fps ~= state.playlist_drawer_loop_fps then
        state.playlist_drawer_loop_timer:kill()
        playlist_drawer_loop_timer_start()
    end
end)

playlist_drawer_set_keys = function(on)
    if on == state.playlist_drawer_keys_on then return end
    if on then
        -- D-pad ↑/↓=长按连续移动选中（complex 绑定，set_key_bindings 的普通绑定不重复）
        mp.add_forced_key_binding("GAMEPAD_DPAD_UP", "zex_drawer_dpad_up",
            playlist_drawer_dpad_event(-1), {complex = true})
        mp.add_forced_key_binding("GAMEPAD_DPAD_DOWN", "zex_drawer_dpad_down",
            playlist_drawer_dpad_event(1), {complex = true})
        mp.enable_key_bindings("playlist_drawer_keys")
    else
        mp.remove_key_binding("zex_drawer_dpad_up")
        mp.remove_key_binding("zex_drawer_dpad_down")
        playlist_drawer_dpad_set(0)
        mp.disable_key_bindings("playlist_drawer_keys")
    end
    state.playlist_drawer_keys_on = on
end

-- 点击抽屉：× 关闭；「定位」按钮跳当前集；行 → 换集
local function playlist_drawer_click()
    -- 动画期间不响应点击：此刻绘制位置和最终位置不一致，点了会错行
    if state.playlist_drawer_anim_start then return end
    if state.playlist_drawer_drag then
        state.playlist_drawer_drag = false
        return
    end
    if state.playlist_drawer_close_hover then
        playlist_drawer_close()
        return
    end
    if state.playlist_drawer_jump_hover then
        playlist_drawer_goto_current()
        return
    end
    -- 季标题行：收起/展开该季，不触发换集；同步键盘/手柄选中到这个季标题
    if state.playlist_drawer_hover_season ~= nil then
        state.playlist_drawer_sel_season = state.playlist_drawer_hover_season
        state.playlist_drawer_sel = nil
        playlist_drawer_toggle_season(state.playlist_drawer_hover_season)
        return
    end
    local i = state.playlist_drawer_hover
    if i ~= nil then
        local rows = playlist_drawer_rows()
        if i >= 0 and i < #rows then
            state.playlist_drawer_sel = i
            state.playlist_drawer_sel_season = nil
            -- 和 Enter 一致：先收起抽屉，动画播完后再切集（避开切集加载抢占动画帧）
            state.playlist_drawer_pending_play = i
            playlist_drawer_close()
        end
    end
...
-- 抽屉文本宽度独立缓存：osc_init 每帧清全局 text_width_cache（切集时 start-file 触发），
-- 若抽屉共用它会动画每帧重新测量全部行文本、拖慢收起；这里独立一份，
-- cache_key 含 style（字体/字号变了 key 也变），无需主动失效
local playlist_drawer_text_w_cache = {}
local function playlist_drawer_estimate_text_width(text, style)
    if text == nil then return 0 end
    text = tostring(text)
    if #text == 0 then return 0 end
    -- replace digits with '0' for consistency
    ...
    local cache_key = measure_text .. (style or "")
    local width = 0
    if playlist_drawer_text_w_cache[cache_key] then
        return playlist_drawer_text_w_cache[cache_key]
    end
    if text_measure_osd and text_measure_osd.update then
        text_measure_osd.res_x = osc_param.playresx
        text_measure_osd.res_y = osc_param.playresy
    ...
    playlist_drawer_text_w_cache[cache_key] = width
    return width
end

-- UTF-8 安全的按字符截断 + 省略号
local function playlist_drawer_truncate(title, max_w, style)
    if playlist_drawer_estimate_text_width(title, style) <= max_w then return title end
    local ell = "…"
    local ell_w = playlist_drawer_estimate_text_width(ell, style)
    local s = title
    while #s > 0 do
        -- 从尾部砍掉一个完整 UTF-8 字符
        local cut = 1
        while cut <= #s and s:byte(#s - cut + 1) < 0xC0 and s:byte(#s - cut + 1) >= 0x80 do
            cut = cut + 1
        end
        s = s:sub(1, #s - cut)
        if playlist_drawer_estimate_text_width(s .. ell, style) <= max_w - ell_w then
            return s .. ell
        end
    end
    return ell
...
-- 一行小工具：填充矩形（圆角可选）
local function playlist_drawer_fill(ass, x1, y1, x2, y2, color, alpha, radius, clip)
    ass:new_event()
    ass:pos(0, 0)
    ass:an(7)
    ass:append(string.format("{\\bord0\\shad0\\1c&H%s&\\1a&H%02X&%s\\p1}", color, alpha, clip or ""))
    ass:draw_start()
...
-- 一行小工具：文字（调用方负责截断，这里只管转义 + 定位）。clip 为 ASS \clip 前缀（可选）
-- 注意 clip 必须包进 {} 才是 tag，否则 libass 把 \clip(...) 当字面文本渲染
local function playlist_drawer_text(ass, x, y, an, style, text, clip)
    ass:new_event()
    ass:pos(x, y)
    ass:an(an)
    if clip then
        ass:append(string.format("{%s}", clip))
    end
    ass:append(style .. mp.command_native({"escape-ass", text}))
end

--
-- 倍速档位悬停面板（ZEX）
-- 悬停 speed 按钮向上滑入档位列表，点击选档；独立 OSD overlay 渲染到显示器
-- 分辨率（物理像素），不受 osd-scale 缩放。几何/命中用 playres 虚拟坐标，
-- 绘制时映射到真实像素。
--
local speed_steps = {2, 1.5, 1.25, 1, 0.75, 0.5}   -- 自上而下：2× 在最上，0.5× 在最下
local speed_panel_w = 68            -- 面板宽：收窄（左缘右移）→ 整体视觉靠右，贴近 speed 按钮
local speed_panel_row_h = 26        -- 档位行高：紧凑排布，档位之间不留大空隙
local speed_panel_pad = 6           -- 面板内边距
local speed_panel_fs = 15
local speed_panel_gap = 6
local speed_panel_close_delay = 0.25

local function speed_panel_rect()
    local e = state.speed_element
    if not (e and e.hitbox) then return 0, 0, 0, 0 end
    local hb = e.hitbox
    -- 水平居中于 speed 按钮正上方（面板中心 = 按钮中心）
    local cx = (hb.x1 + hb.x2) / 2
    local x1 = cx - speed_panel_w / 2
    local x2 = cx + speed_panel_w / 2
    local y2 = hb.y1 - speed_panel_gap
    local y1 = y2 - (#speed_steps * speed_panel_row_h + speed_panel_pad * 2)
    return x1, y1, x2, y2
end

-- 面板是否处于「能接鼠标」的状态：收起动画播放中就不再接管，
-- 光标此时应该重新落回底部栏按钮的普通悬停逻辑
local function speed_panel_active()
    return state.speed_panel_open and not state.speed_panel_closing
end

-- 鼠标在按钮 hitbox ∪ 面板矩形（含 gap 容差）内。按钮判定不依赖面板是否打开，
-- 这样光标停在按钮上时面板保持展开。函数体在此定义，但变量由文件前部
-- 的前向声明接管（render_elements 需要提前调用，见 mouse_hit_coords 上方）
speed_panel_mouse_over = function()
    local e = state.speed_element
    if e and e.hitbox and e.visible ~= false then
        local hb = e.hitbox
        if mouse_hit_coords(hb.x1, hb.y1, hb.x2, hb.y2) then return true end
    end
    if not speed_panel_active() then return false end
    local x1, y1, x2, y2 = speed_panel_rect()
    -- gap 容差：光标从面板往下移到按钮的途中会穿过一条真空带，
    -- 不放大命中区会触发延迟关闭
    return mouse_hit_coords(x1, y1, x2 + 2, y2 + speed_panel_gap + 4)
end

-- 鼠标命中的档位索引（1..#speed_steps），不在面板内返回 nil。
-- 这里用不含容差的严格矩形：gap 容差只用于「保持展开」，不该点出档位
local function speed_panel_hit_index()
    if not speed_panel_active() then return nil end
    local x1, y1, x2, y2 = speed_panel_rect()
    local _, my = get_virt_mouse_pos()
    if not mouse_hit_coords(x1, y1, x2, y2) then return nil end
    local idx = math.floor((my - (y1 + speed_panel_pad)) / speed_panel_row_h) + 1
    if idx < 1 or idx > #speed_steps then return nil end
    return idx
end

-- 面板打开时把 showhide 区扩到整屏（同抽屉 osc_init 里那段），防止光标移上面板
-- （面板整个在底部 deadzone 带之外）触发 mouse_leave 把面板误关。
-- 不备份原值：showhide 区每次 osc_init 都会由 add_area 重算，备份反而会在
-- 期间发生 resize/布局切换时把过期的旧矩形写回去
local function speed_panel_expand_showhide()
    osc_param.areas["showhide"] = {
        {x1 = 0, y1 = 0, x2 = osc_param.playresx, y2 = osc_param.playresy},
    }
end

local function speed_panel_kill_timer()
    if state.speed_panel_timer then
        state.speed_panel_timer:kill()
        state.speed_panel_timer = nil
    end
end

-- 面板彻底关闭：清 overlay + 复位全部状态。
-- 调用方负责按需 request_init（恢复 showhide 区）；osc_init 里调用时不能再
-- 触发重建，否则就是多余的一次布局重建
local function speed_panel_finish_close()
    speed_panel_kill_timer()
    state.speed_panel_open = false
    state.speed_panel_closing = false
    state.speed_panel_hover = nil
    state.speed_panel_anim_start = nil
    state.speed_panel_anim_dir = nil
    state.speed_panel_suppress = false
    if state.speed_overlay.data ~= "" then
        state.speed_overlay.data = ""
        state.speed_overlay:update()
    end
end

-- 面板可见度：无动画 —— 存在即完全显示（p=1），open=false 时不绘制（返回 nil）
-- 悬停出现/选档收起都是直接切换，不做淡入滑入/淡出上移
local function speed_panel_anim_progress()
    if state.speed_panel_open then return 1 end
    return nil
end

local function speed_panel_show()
    speed_panel_kill_timer()
    -- 重新打开时 guard 无意义：清掉，并取消挂起的恢复 timer（防它晚点误触发
    -- request_init 重建 + 覆盖我们刚扩的 showhide 区）
    state.speed_panel_guard = nil
    if state.speed_panel_guard_timer then
        state.speed_panel_guard_timer:kill()
        state.speed_panel_guard_timer = nil
    end
    if speed_panel_active() then return end
    state.speed_panel_open = true
    state.speed_panel_closing = false
    state.speed_panel_hover = nil
    speed_panel_expand_showhide()
    request_tick()
end

-- 直接收起。force=true 时无视光标位置（点选档位后必须收），
-- 否则光标已经移回按钮/面板就取消这次关闭
local function speed_panel_close(force)
    speed_panel_kill_timer()
    if not speed_panel_active() then return end
    if not force and speed_panel_mouse_over() then return end
    -- 无动画：立即清 overlay + 复位状态，并恢复 showhide/捕获区
    speed_panel_finish_close()
    request_init()
end

local function speed_panel_close_delayed()
    if not speed_panel_active() then return end
    if state.speed_panel_timer then return end
    state.speed_panel_timer = mp.add_timeout(speed_panel_close_delay, function()
        state.speed_panel_timer = nil
        speed_panel_close(false)
    end)
end

-- 左键点击按钮 = 开/关。手动收起后置 suppress，防止同一次悬停里
-- 紧接着的 mouse_move 又把面板自动拉回来
local function speed_panel_toggle()
    if speed_panel_active() then
        state.speed_panel_suppress = true
        speed_panel_close(true)
    else
        state.speed_panel_suppress = false
        speed_panel_show()
    end
...
-- 选定档位：设速度 + 强制收起。收起后 suppress 住，光标离开按钮/面板才解除，
-- 否则光标还压在按钮上，下一个 mouse_move 立刻把面板又弹出来
local function speed_panel_pick(idx)
    local v = speed_steps[idx]
    if not v then return end
    mp.commandv("no-osd", "set", "speed", v)
    state.speed = v  -- 立刻更新，别等 observe_property 回调（按钮文字同帧就对）
    state.speed_panel_suppress = true
    speed_panel_close(true)
    -- ZEX：选档后开 guard 缓冲窗口。收起动画播完 open=false 时捕获区恢复底部栏，
    -- 但鼠标还停在原档位位置；紧接着的第二次点击（双击档位 / 快速切换档位）会
    -- 漏成 input.conf 的 MBTN_LEFT cycle pause。guard 期间捕获区保持全屏 + 第二次
    -- 点击走面板逻辑（直接切速 / 重开面板），绝不暂停。0.5s 覆盖 mpv 双击判定(400ms)
    state.speed_panel_guard = mp.get_time() + 0.5
    if state.speed_panel_guard_timer then state.speed_panel_guard_timer:kill() end
    state.speed_panel_guard_timer = mp.add_timeout(0.5, function()
        state.speed_panel_guard = nil
        state.speed_panel_guard_timer = nil
        request_init()  -- 恢复 showhide 区与 input 捕获区（guard 期间被扩成全屏）
    end)
end

-- 每次 mouse_move 调用：在按钮/面板内保持或打开，移出则启动延迟关闭
local function speed_panel_auto()
    if speed_panel_mouse_over() then
        if not state.speed_panel_suppress then speed_panel_show() end
        return true
    end
    state.speed_panel_suppress = false  -- 离开热区，解除抑制
    speed_panel_close_delayed()
    return false
end

-- 虚拟→真实 坐标换算因子（真实像素 = 虚拟坐标 × factor）
local function speed_panel_scale()
    local pw, ph = osc_param.playresx, osc_param.playresy
    local od = state.osd_dimensions
    local dw, dh = od and od.w or 0, od and od.h or 0
    if pw <= 0 or ph <= 0 or dw == 0 or dh == 0 then return 1, 1 end
    return dw / pw, dh / ph
end

-- 一行小工具：填充矩形（圆角可选）。alpha 已按动画进度算好后传进来 ——
-- ASS 里同一 event 内后写的 \1a 会覆盖前面的 \alpha，所以不能靠前缀标签淡出
local function speed_panel_fill(ass, x1, y1, x2, y2, color, alpha, radius)
    ass:new_event()
    ass:pos(0, 0)
    ass:an(7)
    ass:append(string.format("{\\bord0\\shad0\\1c&H%s&\\1a&H%02X&\\p1}", color, alpha))
    ass:draw_start()
    if radius and radius > 0 then
        ass:round_rect_cw(x1, y1, x2, y2, radius)
    else
...
-- 绘制到独立 overlay（物理像素 = 显示器分辨率，不受 osd-scale 缩放）
local function draw_speed_panel()
    local e = state.speed_element
    if not (e and e.hitbox) then
        -- 锚点按钮被布局移除了（窗口缩太窄、布局切换等）：清掉残留的 overlay，
        -- 别让它停在上一次的位置
        speed_panel_finish_close()
        return
    end
    local p = speed_panel_anim_progress()
    if p == nil then return end          -- open=false 时 tick 不会调到这里（防御）
    local x1, y1, x2, y2 = speed_panel_rect()
    if x2 <= x1 then return end
    local fx, fy = speed_panel_scale()
    -- 无动画：直接画在最终位置，不做上移滑入（p 恒 1，slide 恒 0）
    local rx1, ry1 = x1 * fx, y1 * fy
    local rx2, ry2 = x2 * fx, y2 * fy
    local rfs = speed_panel_fs * fy
    local rrow = speed_panel_row_h * fy
    local rpad = speed_panel_pad * fy
    local radius = 6 * fy
    -- 无动画：p 恒 1，fill_a 直接返回 base（固定不透明度），text_alpha 恒全不透明
    local fill_a = function(base) return math.floor(255 - (255 - base) * p) end

    local a = assdraw.ass_new()
    local text_alpha = string.format("&H%02X&", math.floor((1 - p) * 255))
    local accent = osc_color_convert(user_opts.seekbarfg_color)

    -- 背景接近实心（0x12 ≈ 93% 不透明），简洁起见不加左侧 accent 竖线
    speed_panel_fill(a, rx1, ry1, rx2, ry2, "0E0E12", fill_a(0x12), radius)

    local cur = state.speed or 1
    local inner_y = ry1 + rpad
    for i, v in ipairs(speed_steps) do
        local row_y1 = inner_y + (i - 1) * rrow
        local row_y2 = row_y1 + rrow
        local is_cur = math.abs(cur - v) < 0.001
        local is_hover = state.speed_panel_hover == i
        if is_hover or is_cur then
            -- 高亮几乎填满整行（纵向只留 1px 呼吸边），档位之间不露空缺
            speed_panel_fill(a, rx1 + 4 * fx, row_y1 + 1 * fy, rx2 - 4 * fx, row_y2 - 1 * fy,
                is_cur and accent or "FFFFFF", fill_a(is_cur and 0x9A or 0x0F), 4 * fy)
        end
        local color = is_cur and "&HFFFFFF&"
            or (is_hover and ("&H" .. accent .. "&") or "&HAAAAAA&")
        local style = string.format("{\\fs%d\\fn%s\\bord0\\shad0\\1c%s\\1a%s}",
            rfs, user_opts.font, color, text_alpha)
        -- 文字水平居中（an=5）在面板中央，不再贴右缘
        playlist_drawer_text(a, (rx1 + rx2) / 2, (row_y1 + row_y2) / 2, 5, style, string.format("%g×", v))
    end

    local od = state.osd_dimensions
    state.speed_overlay.res_x = od and od.w or 1920
    state.speed_overlay.res_y = od and od.h or 1080
    state.speed_overlay.data = a.text
    state.speed_overlay:update()

    -- 悬停高亮下一帧生效：收起动画期间不再更新（此时不接鼠标）
    state.speed_panel_hover = state.speed_panel_closing and nil or speed_panel_hit_index()
end

-- 绘制抽屉（在 render_element 的 drawer 分支被调用）
-- 注意：不能写成 local function —— 顶部已前向声明，这里赋值给同名局部
draw_playlist_drawer = function(el, ass)
    local bx1, by1, bx2, by2 = playlist_drawer_rect()
    if not bx1 then return end

    -- 滑入/滑出：整体右移 dx。命中判定用的是同一套加了 dx 的坐标，
    -- 否则动画期间悬停高亮会落在终点位置上
    local dx = playlist_drawer_anim_offset()
    local x1, y1, x2, y2 = bx1 + dx, by1, bx2 + dx, by2
    local animating = state.playlist_drawer_anim_start ~= nil

    local mx, my = get_virt_mouse_pos()
    local mouse_ok = mx >= 0 and my >= 0 and not animating

    local pad = playlist_drawer_pad
    local accent = osc_color_convert(user_opts.seekbarfg_color)     -- #00d4ff 强调色
    local font = user_opts.font
    local radius = 12

    local list_y1 = y1 + pad + playlist_drawer_header_h
    local list_y2 = y2 - pad

    local rows = playlist_drawer_rows()
    playlist_drawer_follow_season(rows)
    local entries = playlist_drawer_display()
    local total = #entries
    local visible = playlist_drawer_fit_count(state.playlist_drawer_scroll)
    local max_scroll = playlist_drawer_max_scroll()

    -- 拖滚动条：active_element 期间 render 事件每帧调 playlist_drawer_drag_tick，
    -- 这里只负责把拖拽中的位置钳到合法范围
    state.playlist_drawer_scroll = limit_range(0, max_scroll, state.playlist_drawer_scroll)

    -- ── 悬停状态 ──
    state.playlist_drawer_close_hover = mouse_ok and
        mx >= x2 - 38 and mx <= x2 - 8 and my >= y1 + 7 and my <= y1 + 37
    -- 「定位到当前集」按钮在 × 左边
    state.playlist_drawer_jump_hover = mouse_ok and
        mx >= x2 - 74 and mx <= x2 - 44 and my >= y1 + 7 and my <= y1 + 37

    local hover_row, hover_season = nil, nil
    -- 滚动条存在时它所在的竖带不参与行悬停（避免「悬停滚动条却高亮一行」）
    local scroll_bar_x = x2 - playlist_drawer_pad - 8
    if mouse_ok and mx >= x1 and mx <= x2 and my >= list_y1 and my <= list_y2
        and not state.playlist_drawer_close_hover and not state.playlist_drawer_jump_hover
        and not (max_scroll > 0 and mx >= scroll_bar_x) then
        -- 行高不一（季标题比剧集行矮），不能用固定 pitch 算，逐条量过去找命中；
        -- 落在行间距里不算命中任何行
        local yy = list_y1
        for k = 0, visible - 1 do
            local entry = entries[state.playlist_drawer_scroll + k + 1]
            if not entry then break end
            local eh = playlist_drawer_entry_h(entry)
            if my >= yy and my <= yy + eh then
                if entry.type == "season" then
                    hover_season = entry.season
                else
                    hover_row = entry.row.idx
                end
                break
            end
            yy = yy + eh + playlist_drawer_row_gap
        end
    end
    state.playlist_drawer_hover = hover_row
    state.playlist_drawer_hover_season = hover_season

    -- ── 面板背景 ──
    -- 右侧多画 2×radius，右缘圆角被屏幕裁掉，只留左侧圆角
    playlist_drawer_fill(ass, x1, y1, x2 + radius * 2, y2, "0E0E12", 0x12, radius)
    -- 描边
    ass:new_event()
    ass:pos(0, 0)
    ass:an(7)
    ass:append("{\\bord1\\shad0\\blur0\\1c&H000000&\\1a&HFF&\\3c&HFFFFFF&\\3a&H96&\\p1}")
    ass:draw_start()
    ass:round_rect_cw(x1, y1, x2 + radius * 2, y2, radius)
    ass:draw_stop()

    -- ── 头部 ──
    local hy = y1 + playlist_drawer_header_h / 2
    local watched_total = 0
    for _, row in ipairs(rows) do
        if row.watched then watched_total = watched_total + 1 end
    end
    local header_style = string.format(
        "{\\fs%d\\fn%s\\bord0\\shad0\\1c&HFFFFFF&\\1a&H1A&}", playlist_drawer_header_fs, font)
    playlist_drawer_text(ass, x1 + pad + 6, hy, 4, header_style, "播放列表")
    -- 数量：有边车数据时顺带报已看进度（分母用剧集总数，不含季标题行）
    local count_text = playlist_meta.items
        and string.format("%d/%d", watched_total, #rows)
        or string.format("%d", #rows)
    local count_style = string.format(
        "{\\fs%d\\fn%s\\bord0\\shad0\\1c&H%s&\\1a&H28&}", playlist_drawer_sub_fs, font, accent)
    playlist_drawer_text(
        ass,
        x1 + pad + 6 + playlist_drawer_estimate_text_width("播放列表", header_style) + 10,
        hy + 1, 4, count_style, count_text)

    -- 「定位到当前集」（用播放图标；modernz-icons 没有 target 类字形）
    playlist_drawer_text(ass, x2 - 59, hy, 5,
        string.format("{\\fs15\\fn%s\\bord0\\shad0\\1c&H%s&\\1a&H%02X&}", icons.iconfont,
            state.playlist_drawer_jump_hover and accent or "FFFFFF",
            state.playlist_drawer_jump_hover and 0x14 or 0x50),
        icons.playlist)
    -- × 关闭
    playlist_drawer_text(ass, x2 - 23, hy, 5,
        string.format("{\\fs15\\fn%s\\bord0\\shad0\\1c&H%s&\\1a&H14&}", icons.iconfont,
            state.playlist_drawer_close_hover
                and osc_color_convert(user_opts.windowcontrols_close_hover) or "FFFFFF"),
        icons.window.close)
    -- 分隔线
    playlist_drawer_fill(ass, x1 + pad, list_y1 - 3, x2 - pad, list_y1 - 2, "FFFFFF", 0x8C, 0)

    -- ── 行 ──
    local rx1, rx2 = x1 + pad, x2 - pad - 8      -- 右边留出滚动条的位置
    local num_w = 34                              -- 集号栏宽度
    local text_x = rx1 + 10 + num_w
    local max_w = rx2 - text_x - 12
    local watched_green = osc_color_convert("#4ADE80")   -- 已看标记的柔和绿
    -- 列表区裁剪：滚动时首/末条部分越出 [list_y1, list_y2]，clip 保证不画到头部/列表外
    local row_clip = string.format("\\clip(%d,%d,%d,%d)",
        math.floor(rx1), math.floor(list_y1), math.floor(rx2), math.floor(list_y2))
    ...
    for k = 0, visible - 1 do
        ...
        if entry.type == "season" then
            -- ── 季标题行：▼/▶ + 第 N 季 + 已看统计，点击收起/展开 ──
            local hy1 = yy
            local hy2 = yy + playlist_drawer_season_h
            local collapsed = state.playlist_drawer_collapsed[entry.season] == true
            local hov = state.playlist_drawer_hover_season == entry.season
            local sel_s = state.playlist_drawer_sel_season == entry.season
            if sel_s then
                playlist_drawer_fill(ass, rx1, hy1 + 5, rx1 + 3, hy2 - 5, accent, 0x00, 1.5, row_clip)
            elseif hov then
                playlist_drawer_fill(ass, rx1, hy1 + 5, rx1 + 3, hy2 - 5, "FFFFFF", 0x80, 1.5, row_clip)
            end
            local mid_y = (hy1 + hy2) / 2
            -- 收起/展开箭头（U+25B6/U+25BC，正文字体自带）
            playlist_drawer_text(ass, rx1 + 12, mid_y, 4,
                string.format("{\\fs%d\\fn%s\\bord0\\shad0\\1c&H%s&\\1a&H%02X&}",
                    playlist_drawer_sub_fs, font, accent, hov and 0x00 or 0x28),
                collapsed and "▶" or "▼", row_clip)
            playlist_drawer_text(ass, rx1 + 28, mid_y, 4,
                string.format("{\\fs%d\\fn%s\\bord0\\shad0\\b1\\1c&H%s&\\1a&H0A&}",
                    playlist_drawer_fs - 1, font, accent),
                string.format("第 %d 季", entry.season), row_clip)
            -- 右侧统计；收起时补一句共几集，让人知道里面装着什么
            local meta = string.format("%d/%d 已看", entry.watched, entry.count)
            if collapsed then meta = string.format("共 %d 集 · ", entry.count) .. meta end
            playlist_drawer_text(ass, rx2 - 12, mid_y, 6,
                string.format("{\\fs%d\\fn%s\\bord0\\shad0\\1c&HFFFFFF&\\1a&H66&}",
                    playlist_drawer_sub_fs, font),
                meta, row_clip)
            yy = hy2 + playlist_drawer_row_gap
        else
            local row = entry.row
            local i = row.idx
            local ry1 = yy
            local ry2 = yy + playlist_drawer_row_h
            local selected = (state.playlist_drawer_sel == i)

            -- 左缘竖条标记：当前集/键盘选中用青、悬停用白淡竖条，零色块
            if row.current or selected then
                playlist_drawer_fill(ass, rx1, ry1 + 5, rx1 + 3, ry2 - 5, accent, 0x00, 1.5, row_clip)
            elseif i == hover_row then
                playlist_drawer_fill(ass, rx1, ry1 + 5, rx1 + 3, ry2 - 5, "FFFFFF", 0x80, 1.5, row_clip)
            end

            -- 第一行：集号 + 单集标题。集号永远显示（已看也要知道是 E 几）；
            -- 已看的集在右侧补一个绿色 ✓，标题只略压暗 —— 一眼认出「看完了」，
            -- 而不是整行灰掉像被禁用
            local num_text
            if row.episode then
                num_text = string.format("E%02d", row.episode)
            else
                num_text = string.format("%02d", i + 1)
            end
            playlist_drawer_text(ass, rx1 + 12, ry1 + 17, 4,
                string.format("{\\fs%d\\fn%s\\bord0\\shad0\\1c&H%s&\\1a&H10&}",
                    playlist_drawer_fs, font, accent),
                num_text, row_clip)
            if row.watched then
                playlist_drawer_text(ass, rx2 - 12, ry1 + 17, 6,
                    string.format("{\\fs13\\fn%s\\bord0\\shad0\\b1\\1c&H%s&}", font, watched_green),
                    "✓", row_clip)
            end

            -- 标题颜色：当前集白；键盘选中非当前集变青（配左缘青竖条）；已看且非当前集压暗
            local t_color = "FFFFFF"
            local title_alpha = 0x00
            if selected and not row.current then
                t_color = accent
            elseif row.watched then
                title_alpha = 0x4D
            end
            local t_style = string.format("{\\fs%d\\fn%s\\bord0\\shad0\\1c&H%s&\\1a&H%02X&}",
                playlist_drawer_fs, font, t_color, title_alpha)
            -- 已看行右侧有 ✓，标题少占 18px 避免顶到
            local row_max_w = row.watched and (max_w - 18) or max_w
            playlist_drawer_text(ass, text_x, ry1 + 17, 4, t_style,
                playlist_drawer_truncate(row.title or "", row_max_w, t_style), row_clip)

            -- 第二行：时长 · 已看完 / 观看进度
            local sub_parts = {}
            if row.runtime and row.runtime > 0 then
                sub_parts[#sub_parts + 1] = string.format("%d 分钟", row.runtime)
            end
            if row.watched then
                sub_parts[#sub_parts + 1] = "已看完"
            elseif row.runtime and row.runtime > 0 and (row.watched_ms or 0) > 0 then
                local pct = (row.watched_ms / 1000 / 60) / row.runtime * 100
                sub_parts[#sub_parts + 1] = string.format("已看 %d%%", limit_range(0, 100, pct))
            end
            if #sub_parts > 0 then
                playlist_drawer_text(ass, text_x, ry2 - 13, 4,
                    string.format("{\\fs%d\\fn%s\\bord0\\shad0\\1c&H%s&\\1a&H%02X&}",
                        playlist_drawer_sub_fs, font,
                        row.watched and watched_green or "FFFFFF",
                        row.watched and 0x20 or 0x66),
                    table.concat(sub_parts, "  ·  "), row_clip)
            end

            -- 底部进度条：已看完画一整条淡绿；看到一半画部分进度
            if row.runtime and row.runtime > 0 then
                local bar_x2 = rx2 - 12
                if row.watched then
                    playlist_drawer_fill(ass, text_x, ry2 - 5, bar_x2, ry2 - 3, watched_green, 0x4D, 1, row_clip)
                elseif (row.watched_ms or 0) > 0 then
                    local frac = limit_range(0, 1, (row.watched_ms / 1000 / 60) / row.runtime)
                    playlist_drawer_fill(ass, text_x, ry2 - 5, bar_x2, ry2 - 3, "FFFFFF", 0xC0, 1, row_clip)
                    playlist_drawer_fill(ass, text_x, ry2 - 5,
                        text_x + (bar_x2 - text_x) * frac, ry2 - 3, accent, 0x20, 1, row_clip)
                end
            end
            yy = ry2 + playlist_drawer_row_gap
        end
    end

    -- ── 滚动条 ──
    if max_scroll > 0 then
        local bar_x = x2 - pad + 2
        local track_h = list_y2 - list_y1
        local thumb_h = math.max(24, track_h * visible / total)
        -- 滚动条跟随整数滚动位置
        local thumb_y = list_y1 + (track_h - thumb_h) * state.playlist_drawer_scroll / max_scroll
        playlist_drawer_fill(ass, bar_x, list_y1, bar_x + 3, list_y2, "FFFFFF", 0xE0, 1.5)
        playlist_drawer_fill(ass, bar_x, thumb_y, bar_x + 3, thumb_y + thumb_h,
            state.playlist_drawer_drag and accent or "FFFFFF",
            state.playlist_drawer_drag and 0x20 or 0x60, 1.5)
    end
end
...
local function osc_init()
    ...
    -- ZEX：布局重建（切集、窗口尺寸变化、设置修改）时倍速面板的锚点
    -- speed_element 会被 prepare_elements 重建而失效，直接干净收掉面板，
    -- 否则残留的 overlay 会停在上一次的位置。这里不再 request_init，
    -- 我们本来就在初始化流程里
    speed_panel_finish_close()
    ...
    -- ZEX：固定全屏播放，只按全屏缩放
    local scale = user_opts.scalefullscreen
    ...
    -- ZEX：不再有最大化/还原按钮 —— 右上角不再提供切换全屏状态的入口
    ...
    ne.content = function ()
        -- ZEX：直接读播放列表当前项的标题（m3u EXTINF 的中文剧集标题）。
        -- 不用 media-title —— 它优先取文件内嵌 title（压制组写的 "MapoTofu压制" 之类）
        local pos = mp.get_property_number("playlist-pos", -1)
        local t = (pos >= 0) and mp.get_property("playlist/" .. pos .. "/title") or nil
        if not t or t == "" then
            t = mp.get_property("media-title") or ""
        end
        t = make_escaped_title(t)
    ...
    ne.eventresponder["mbtn_left_up"] = function () mp.commandv("seek", -jump_amount, jump_mode) end
    ne.eventresponder["mbtn_right_up"] = function () mp.commandv("seek", -jump_more_amount, jump_mode) end
    ...
    ne.eventresponder["mouse_move"] = function (element)
        ...
        if element.state.lastseek == nil or element.state.lastseek ~= setvol then
                mp.commandv("no-osd", "set", "volume", setvol)
    ...
    ne.eventresponder["mbtn_left_down"] = function (element)
        ...
        mp.commandv("no-osd", "set", "volume", set_volume(pos))
    end
    ...
    -- ZEX：循环按钮自定处理 —— 左键切换文件循环，并像音量提示一样在左上角
    -- 显示中文小字提示（脚本消息发给 volume_osd.lua 的 notify）；右键无功能
    ne.eventresponder["mbtn_left_up"] = function()
        mp.command("no-osd cycle-values loop-file inf no")
        -- 用字符串读取：inf/yes = 开，no = 关（bool 转换对 "inf" 不可靠，会恒判为关）
        local loop_str = mp.get_property("loop-file", "no")
        mp.commandv("script-message-to", "volume_osd", "notify",
            "循环：" .. ((loop_str ~= "no") and "开启" or "关闭"))
    end
    ne.eventresponder["mbtn_right_up"] = function() end
    ...
    bind_buttons("speed")
    -- ZEX：倍速改为悬停弹出档位面板，左键 = 开/关面板（右键/滚轮命令已全 ignore）
    ne.eventresponder["mbtn_left_up"] = speed_panel_toggle
    ...
    -- ZEX：播放列表侧边抽屉（关闭时 visible=false，prepare_elements 会移除它，
    -- 不影响视频区鼠标事件；开关由 playlist_drawer_toggle 触发 request_init）
    ne = new_element("playlist_drawer", "box")
    ne.visible = state.playlist_drawer_visible
    ne.eventresponder["mbtn_left_up"] = playlist_drawer_click
    ne.eventresponder["mbtn_left_down"] = playlist_drawer_start_drag
    ne.eventresponder["wheel_up_press"] = function() playlist_drawer_scroll_by(-3) end
    ne.eventresponder["wheel_down_press"] = function() playlist_drawer_scroll_by(3) end
    -- 滚动条拖拽：active_element 期间 render 事件每帧调用（draw_playlist_drawer
    -- 里按鼠标位置算 scroll），reset 事件在 mouse-up 时清理拖拽状态
    ne.eventresponder["render"] = playlist_drawer_drag_tick
    ne.eventresponder["reset"] = function()
        state.playlist_drawer_drag = false
    end
    local dlo = add_layout("playlist_drawer")
    dlo.geometry = {x = osc_param.playresx, y = 66, an = 9, w = playlist_drawer_w,
                    h = math.max(200, osc_param.playresy - 66 - 130)}
    dlo.style = ""
    dlo.group = "drawer"
    dlo.layer = 30    -- 低于底部按钮(50)/进度条(49)：重叠区域底部控件优先

    -- ZEX：抽屉开着时把 showhide 区扩到整屏。
    -- showhide section 只绑 mouse_move/mouse_leave，它的区域默认是底部栏那条
    -- deadzone 带（deadzonesize=0.95 → 只有屏幕最底部约 5%）。抽屉在右侧中段，
    -- 光标一移上去就离开该区 → mpv 发 mouse_leave → state.mouse_in_window=false
    -- → get_virt_mouse_pos() 返回 -1,-1 → 悬停判定和 refresh_input_area 的
    -- mouse_hit 全失败：抽屉既没有悬停反馈，点击也穿透成「暂停/继续」。
    -- 注意 mp.set_mouse_area 每个 section 只认一个矩形（render 里的循环多次调用
    -- 只有最后一次生效），所以必须整个替换而不是 add_area 追加
    if state.playlist_drawer_visible then
        osc_param.areas["showhide"] = {
            {x1 = 0, y1 = 0, x2 = osc_param.playresx, y2 = osc_param.playresy},
        }
    end

    -- ZEX：倍速面板 guard 缓冲窗口期间保持 showhide 全屏。guard 时 open=false、
    -- 上面 add_area 已把 showhide 重算回底部栏那条 deadzone 带，而鼠标还停在原
    -- 面板位置（deadzone 之外）→ 会立刻触发 mouse_leave → deadzone_hide=instant
    -- → hide_osc() 清空 input 捕获区 → guard 的全屏捕获被拆掉。必须整屏替换
    if state.speed_panel_open or
        (state.speed_panel_guard and mp.get_time() < state.speed_panel_guard) then
        speed_panel_expand_showhide()
    end
    ...
    state.playlist_drawer_element = elements["playlist_drawer"]
    state.speed_element = elements["speed"]
...
local function hide_osc()
    ...
    -- ZEX：隐藏时取消进度条预览延迟，避免 timer/就绪态残留
    if state.seek_thumb_timer then
        state.seek_thumb_timer:kill()
        state.seek_thumb_timer = nil
    end
    ...
    -- ZEX：抽屉开着时不能清 —— 抽屉自己要接点击（点行换集/点空白关抽屉），
    -- 清了会漏给 input.conf 的 MBTN_LEFT cycle pause
    -- 倍速面板同理：开着时面板要接点击，且清掉会让 run_autohide 认为光标
    -- 不在 input 区 → 底部栏被收起
    if not state.playlist_drawer_visible and not state.speed_panel_open and
        not (state.speed_panel_guard and mp.get_time() < state.speed_panel_guard) then
...
local function mouse_leave()
    -- ZEX：光标真离开窗口，guard 缓冲窗口不再需要 —— 清掉并取消挂起的恢复 timer，
    -- 否则它晚点触发 request_init 会无谓重建布局
    state.speed_panel_guard = nil
    if state.speed_panel_guard_timer then
        state.speed_panel_guard_timer:kill()
        state.speed_panel_guard_timer = nil
    end
    -- ZEX：倍速面板开着时 showhide 区已扩到整屏，能收到 mouse_leave 说明光标真
    -- 离开了窗口，此时收掉面板再走正常流程。收起动画播放中也直接收干净，
    -- 否则 out 动画播完后 cursor 已不在窗口，面板会卡在残留状态
    state.speed_panel_suppress = false
    if state.speed_panel_open then
        if speed_panel_mouse_over() then
            speed_panel_close(true)
        else
            speed_panel_finish_close()
    ...
    -- ZEX：抽屉开着时 showhide 区已扩到整屏，能收到这个事件说明光标真的离开了窗口。
    -- 但此时不该收起底部栏 —— 抽屉还开着，收了底部栏会让「列表按钮」跟着消失
    if state.playlist_drawer_visible then
    ...
    state.last_mouseX, state.last_mouseY = nil, nil
    state.mouse_in_window = false
    -- ZEX：位置语义变了（窗口外 → -1,-1），缓存立即失效
    state.mouse_cache_x, state.mouse_cache_y = nil, nil
end
...
local function handle_touch(_, touchpoints)
    ...
    if touchpoints then
        ...
        if #touchpoints > 0 then
            ...
            -- ZEX：触摸点即鼠标位置来源（get_virt_mouse_pos 的 recently_touched 分支），失效缓存
            state.mouse_cache_x, state.mouse_cache_y = nil, nil
        end
    end
end
...
local function refresh_input_area()
    -- ZEX：倍速面板打开/收起期间保持全屏捕获（必须在 osc_visible 判定之前）：
    -- 面板画在底部栏上方、大部分在 input 区（底部栏那条）之外，若按底部栏的
    -- input 区判定，光标停在面板上时 run_autohide 会以为光标离开控制区 →
    -- 1.5s 后整条底部栏被收走，倍速按钮连同数字一起消失。
    -- 全屏捕获让点面板外空白 = 收起面板（process_event 的 up 分支接住，不漏成暂停）
    -- ZEX：guard 缓冲窗口期间保持全屏捕获 —— 选档收起后 open=false，但鼠标还停在
    -- 原档位位置，捕获区若恢复底部栏，紧接着的第二次点击（双击档位/快速切档）会
    -- 漏给 input.conf 的 MBTN_LEFT cycle pause
    if state.speed_panel_open or
        (state.speed_panel_guard and mp.get_time() < state.speed_panel_guard) then
        set_virt_mouse_area(0, 0, osc_param.playresx, osc_param.playresy, "input")
        set_virt_mouse_area(0, 0, osc_param.playresx, osc_param.playresy, "input_wheel")
        set_virt_mouse_area(0, 0, osc_param.playresx, osc_param.playresy, "input_mid")
        return
    end

    -- 抽屉开着时独立于底部栏保持捕获：底部栏隐藏时抽屉仍可点/滚
    local drawer_active = state.playlist_drawer_visible and
        state.playlist_drawer_element and state.playlist_drawer_element.hitbox
    if not state.osc_visible and not drawer_active then
        set_virt_mouse_area(0, 0, 0, 0, "input")
    ...
    -- 抽屉开着：左键捕获扩到全屏。点抽屉外空白 = 关抽屉（process_event 的 up 分支），
    -- 不能漏给 input.conf 的 MBTN_LEFT cycle pause —— 抽屉刚滑入、鼠标还没动过时，
    -- 捕获区还停在底部栏按钮上，这时点列表会直接触发播放/暂停
    if drawer_active then
        set_virt_mouse_area(0, 0, osc_param.playresx, osc_param.playresy, "input")
        local d = state.playlist_drawer_element.hitbox
        set_virt_mouse_area(d.x1, d.y1, d.x2, d.y2, "input_wheel")
        set_virt_mouse_area(d.x1, d.y1, d.x2, d.y2, "input_mid")
        return
    end
    ...
    if not mouse_in_area("input") and not (drawer_active and mouse_hit(state.playlist_drawer_element)) then
        set_virt_mouse_area(0, 0, 0, 0, "input")
        set_virt_mouse_area(0, 0, 0, 0, "input_wheel")
        set_virt_mouse_area(0, 0, 0, 0, "input_mid")
        return
    ...
    local areas = osc_param.areas["input"] or {}
    if hovered_click then
        set_virt_mouse_area(
            hovered_click.hitbox.x1, hovered_click.hitbox.y1,
            hovered_click.hitbox.x2, hovered_click.hitbox.y2, "input")
    elseif #areas > 0 then
        -- 控制栏空白也保持捕获：否则点击漏给 input.conf 的 MBTN_LEFT cycle pause
        local x1, y1, x2, y2 = areas[1].x1, areas[1].y1, areas[1].x2, areas[1].y2
        for i = 2, #areas do
            x1 = math.min(x1, areas[i].x1)
            y1 = math.min(y1, areas[i].y1)
            x2 = math.max(x2, areas[i].x2)
            y2 = math.max(y2, areas[i].y2)
        end
        set_virt_mouse_area(x1, y1, x2, y2, "input")
    else
        set_virt_mouse_area(0, 0, 0, 0, "input")
    end
    set_virt_mouse_area(
...
local function process_event(source, what)
    -- ZEX：事件入口失效鼠标缓存。事件可能在两次 tick 之间到达，且 mouse_move 分支会先改
    -- state.mouse_in_window 再取坐标 —— 不失效的话鼠标刚进窗口那一帧会读到旧的 -1,-1
    state.mouse_cache_x, state.mouse_cache_y = nil, nil
    ...
    if what == "down" or what == "press" then
        ...
        -- ZEX：倍速面板弹出且鼠标在面板/按钮热区内时，底部栏元素的点击交给面板
        -- 逻辑处理。面板是独立 overlay，不参与元素 hitbox —— 鼠标按在面板上时
        -- seekbar 等被遮挡元素的命中判定依然成立，会把点击/拖动误发给进度条。
        -- 只放行 speed 按钮本身（它在热区内，点击 = 开/关面板）
        local panel_block = state.speed_panel_open and speed_panel_mouse_over()
        for n = 1, #elements do
            if (not panel_block or elements[n].name == "speed")
                and mouse_hit(elements[n]) and elements[n].eventresponder then
                hovered_any = true
                if (elements[n].eventresponder[source .. "_up"] or
        ...
        -- ZEX：滚轮在视频空白区域（没悬停在任何控件上）= 调音量。
        -- 用 no-osd 避免中央弹出音量条；音量变化由 volume_osd.lua 在左上角提示
        if not handled and not hovered_any and
            (source == "wheel_up" or source == "wheel_down") then
            mp.command(("no-osd add volume %s"):format(source == "wheel_up" and 5 or -5))
        end
    elseif what == "up" then
        ...
        elseif source == "mbtn_left" then
            if speed_panel_active() then
                if state.speed_panel_anim_start then
                    -- 面板还在滑入，不响应点击（抽屉同款）
                else
                    local idx = speed_panel_hit_index()
                    if idx then
                        speed_panel_pick(idx)         -- 点档位生效并收起
                    elseif not speed_panel_mouse_over() then
                        speed_panel_toggle()          -- 点面板外空白 = 收起，不暂停
                    end
                end
            elseif state.speed_panel_open then
                -- 收起动画播放中：不响应点击，防漏成下面的暂停/继续
            elseif state.speed_panel_guard and mp.get_time() < state.speed_panel_guard then
                -- ZEX：面板刚收起（guard 缓冲窗口）。open=false 但 input 捕获区仍全屏，
                -- 这次点击（双击档位的第二次 up，或快速切换档位的第二次点击）不能当成
                -- "单击视频空白"去暂停。点在原档位行上 = 直接切速生效（guard 续期）；
                -- 点在面板/按钮热区 = 重开面板；其它位置 = 忽略
                local gx1, gy1, gx2, gy2 = speed_panel_rect()
                if mouse_hit_coords(gx1, gy1, gx2, gy2) then
                    local _, gy = get_virt_mouse_pos()
                    local gidx = math.floor((gy - (gy1 + speed_panel_pad)) / speed_panel_row_h) + 1
                    if gidx >= 1 and gidx <= #speed_steps then
                        speed_panel_pick(gidx)
                    else
                        state.speed_panel_suppress = false
                        speed_panel_show()
                    end
                elseif speed_panel_mouse_over() then
                    state.speed_panel_suppress = false
                    speed_panel_show()
                end
            elseif state.playlist_drawer_visible and not state.playlist_drawer_closing then
                -- ZEX：抽屉开着时，点抽屉外空白 = 关闭抽屉，不切暂停
                if not playlist_drawer_mouse_over() then
                    playlist_drawer_close()
                end
            else
                -- ZEX：单击视频空白处 = 暂停/继续（PotPlayer 习惯）。
                -- 控制栏显示时，点其区域（含控件间空白、进度条）不算视频空白，不触发暂停
                if not (state.osc_visible and mouse_in_area("input")) then
                    mp.commandv("cycle", "pause")
                end
            end
        end
    ...
    elseif source == "mouse_move" then
        ...
        -- ZEX：光标在抽屉上时不跑底部栏的 deadzone 判定。抽屉开着时 showhide 区
        -- 被扩成了整屏，不跳过的话在抽屉上移动会一直命中 showhide → 反复拉起底部栏
        -- ZEX：倍速面板：悬停按钮打开、移出延迟关闭；光标在面板/按钮上时跳过
        -- 底部栏 deadzone 判定（showhide 已扩全屏，在面板上移动会命中 showhide）
        local over_speed = speed_panel_auto()
        if over_speed then
        ...
        local over_drawer = playlist_drawer_mouse_over()
        if over_drawer then
            -- 抽屉本身要跟着光标重绘（悬停行高亮）
            state.last_mouseX, state.last_mouseY = mouseX, mouseY
            local n = state.active_element
...
local function render()
    ...
    if (state.osc_visible or state.playlist_drawer_visible) ~= state.input_enabled then
        if state.osc_visible or state.playlist_drawer_visible then
            mp.enable_key_bindings("input")
        else
            mp.disable_key_bindings("input")
        end
        state.input_enabled = state.osc_visible or state.playlist_drawer_visible
    end
    refresh_input_area()

    -- ZEX：全隐藏早退。控制条/窗口控件/抽屉都不可见、无进行中的动画、没有按住的元素、
    -- 也不画常驻进度条时，下面全是白算 —— 算完布局、autohide、元素几何，最后交给
    -- set_osd 的是空串，被它的去重判断丢掉。播放中 tick 按刷新率跑（300Hz 屏每秒 300 次），
    -- 正常看片时控制条恰恰一直是隐藏的，这一段是常态开销。
    --
    -- 早退点必须在 showhide 区设置 + do_enable_keybindings + refresh_input_area 之后：
    -- 前两者负责「鼠标移到底部能唤回控制条」，跳过就再也叫不出来；后者维持点击捕获区。
    -- 放行条件里 animation/anitype 保证淡入淡出过程照常渲染，active_element 保证
    -- 按住拖动进度条时不断流
    if not state.osc_visible and not state.wc_visible and not state.playlist_drawer_visible
        and not state.animation and state.anitype == nil
        and not state.wc_animation and state.wc_anitype == nil
        and not state.active_element and not state.speed_panel_open
        and not (user_opts.persistent_progress or state.persistent_progress_toggle)
    then
        -- 上一帧可能还有残留内容（刚淡出完），交一次空串把 OSD 清干净；set_osd 内部去重，
        -- 稳态下不会真的触发 update
        set_osd(state.osd, osc_param.playresy * osc_param.display_aspect, osc_param.playresy, "", 1000)
        return
    end
    ...
    -- autohide
    local function run_autohide(showtime_key, hide_fn, input_areas, visible_key)
        ...
        if timeout <= 0 and get_touchtimeout() <= 0 then
            ...
            -- ZEX：倍速面板开着（或 guard 缓冲窗口内）且光标在面板/按钮热区内时禁止
            -- 自动收起底部栏：run_autohide 靠 mouse_in_area("input") 判断光标，而面板
            -- 大部分画在 input 区（底部栏那条）上方，直接判断会误以为光标离开控制区
            -- → 1.5s 后整条栏（含倍速按钮和数字）被收走
            local speed_blocks_hide = (state.speed_panel_open or
                (state.speed_panel_guard and mp.get_time() < state.speed_panel_guard))
                and speed_panel_mouse_over()
            if not element_blocks_hide and not speed_blocks_hide
                and (not user_opts.keep_with_cursor or not mouse_in_area(input_areas)) then
                -- ZEX：已经藏过的栏别反复 hide —— hide_osc 每次调用都有副作用
                -- （清输入捕获区），反复调会把捕获区压死在清空状态
                if visible_key == nil or state[visible_key] then
                    hide_fn()
                end
            end
        else
    ...
    if not state.keeponpause_active then
        run_autohide("showtime", hide_osc, osc_areas, "osc_visible")
    end
    run_autohide("wc_showtime", hide_wc, wc_areas, "wc_visible")

    -- actual rendering
    local ass = assdraw.ass_new()

    if state.osc_visible or state.wc_visible or state.playlist_drawer_visible then
        render_elements(ass, state.osc_visible, state.wc_visible)
    end

    -- ZEX：倍速档位面板画到独立 overlay（物理像素 = 显示器分辨率，不受 osd-scale 缩放）
    if state.speed_panel_open then
        draw_speed_panel()
    end
    ...
    -- 抽屉独立 overlay：空闲时（无滚动/动画自驱动循环）由这里补一次绘制，
    -- 悬停高亮、布局/尺寸变化后的重绘都走它；滚动/动画中抽屉循环自己画。
    if state.playlist_drawer_visible and not state.playlist_drawer_loop_active then
        -- ZEX：抽屉开着时定期回读边车（queue.json）。ZEX 换集/标记已看会重写它，
        -- 但原先只在打开抽屉那一刻 load 一次 —— 开着看的话勾和进度条一直是旧的，
        -- 得关掉重开才刷新。load_meta 内部按 mtime 比对，没变就直接返回（只一次
        -- file_info），所以这里 0.5s 探一次开销可忽略；变了才真正重读 + 重建行缓存
        local now_t = mp.get_time()
        if not state.playlist_meta_poll or now_t - state.playlist_meta_poll >= 0.5 then
            state.playlist_meta_poll = now_t
            playlist_drawer_load_meta()
        end
        playlist_drawer_draw_overlay()
    end
...
tick = function()
    -- ZEX：新的一帧，鼠标位置缓存失效（见 get_virt_mouse_pos）
    state.mouse_cache_x, state.mouse_cache_y = nil, nil
    ...
    elseif state.fullscreen and user_opts.showfullscreen then
    ...
    -- 右摇杆滚动已迁到抽屉独立循环（playlist_drawer_loop_tick：真实 dt 推进 +
    -- 超时兜底），不再走 OSC 的共享 tick——滚动时不会连带整条 OSC 每帧重绘。
end

-- 读取有效显示器刷新率：display-fps 优先，estimated-display-fps 兜底。
-- display-fps 可能为 nil（VO 未报告）或 0（VO 报无效值）——Lua 里 0 是 truthy，
-- 直接 `not` 判断会漏掉 0，走到 1/0 = inf 把 tick 定时器冻住，故统一 <=0 判断。
-- estimated-display-fps 按实际渲染间隔估算，display-resample 模式下很准。
effective_display_fps = function()
...
local function set_tick_delay()
    if not user_opts.tick_delay_follow_display_fps then
        tick_delay = user_opts.tick_delay
        return
    end
    local display_fps = effective_display_fps()
    if not display_fps then
        -- 两个刷新率属性都无效（VO 未就绪 / 不报告）：保留上次跟随值，
        -- 菜单不掉回固定 60；display/estimated 生效时 observe 会再触发更新
        return
    end
...
mp.observe_property("estimated-display-fps", "number", set_tick_delay)
...
-- ZEX：loop-file 是字符串属性（no/inf/yes），bool 观察对 "inf" 转换不可靠，
-- 用 string 观察并显式判断：inf/yes = 开，no = 关
mp.observe_property("loop-file", "string", function(_, val)
    state.file_loop = (val ~= nil and val ~= "no")
end)

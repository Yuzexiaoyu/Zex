...
import { Play, Info, Trash2, ImagePlus, RectangleHorizontal, HardDrive, FolderOpen, Gauge } from 'lucide-react';
...
interface Props {
  ...
  onFpsSettings: () => void;   // 打开帧数面板并选中这个游戏
}

export default function GameContextMenu({ game, x, y, onClose, onChangeCover, onChangeBanner, onDiskManage, onFpsSettings }: Props) {
  const t = useT();
  ...
  useModalGamepad('menu:game', {
    ...
    activate: (i) => {
      ...
      else if (i === 5) handleFpsSettings();
      else if (i === 6) handleDiskManage();
  ...
  // 帧数设置：入口唯一 —— 全局和单游戏都在那个面板里改（这里只负责选中本游戏）
  const handleFpsSettings = () => {
    onClose();
    onFpsSettings();
  };
  ...
  return createPortal(
    ...
    >
      <div className="glass-card w-44 py-1.5 animate-scale-in">
        ...
        <button onClick={handleFpsSettings} className={clsx('context-menu-item', focused === 5 && 'gamepad-focus')}>
          <Gauge size={14} className="text-text-secondary" />
          {t('games.fpsSettingsItem')}
        </button>
        <button onClick={handleDiskManage} className={clsx('context-menu-item', focused === 6 && 'gamepad-focus')}>
          <HardDrive size={14} className="text-text-secondary" />
          {t('games.diskManage')}
        </button>
        <div className="context-menu-divider" />
        <button onClick={handleDelete} className={clsx('context-menu-item text-danger', focused === 7 && 'gamepad-focus')}>
          <Trash2 size={14} />
          {t('games.deleteGame')}
        </button>
      </div>

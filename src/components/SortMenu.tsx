...
export interface SortOption {
  key: string;
  /** i18n key：由 SortMenu 内部经 t() 渲染（传中文原文时 t 兜底原样返回，安全） */
  label: string;
  hint?: string;
  ...
  return (
    <>
      <button
        ...
        title={t('common.selectSort')}
      >
        <ArrowUpDown size={14} />
        <span className="text-text-tertiary">{t('common.sort')}</span>
        <span className="font-medium text-text-primary">{current ? t(current.label) : ''}</span>
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0 text-left">
                    <span className="block text-[13px] font-medium leading-tight">{t(o.label)}</span>
                    {o.hint && (
                      <span className="block mt-0.5 text-[11px] text-text-tertiary leading-tight">{t(o.hint)}</span>
                    )}
                  </span>

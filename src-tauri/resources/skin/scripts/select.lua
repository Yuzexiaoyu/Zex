...
mp.add_key_binding(nil, "select-track", function ()
    ...
    for i, track in ipairs(mp.get_property_native("track-list")) do
        tracks[i] = (track.image and "图像" or
                     track.type:sub(1, 1):upper() .. track.type:sub(2)) .. ": " ..
                    format_track(track)
    end

    if #tracks == 0 then
        show_warning("没有可用的轨道。")
        return
    end
...
mp.add_key_binding(nil, "select-edition", function ()
    ...
    for i, edition in ipairs(edition_list) do
        editions[i] = edition.title or ("版本 " .. edition.id + 1)
    end
...
mp.add_key_binding(nil, "select-watch-history", function ()
    ...
    if not history_file then
        show_warning(mp.get_property_native("save-watch-history")
                     and error_message
                     or "启用 --save-watch-history 可跳转到最近播放的文件。")
        return
    end
...
mp.add_key_binding(nil, "select-watch-later", function ()
    ...
    if #files == 0 then
        show_warning(mp.get_property_native("write-filename-in-watch-later-config")
            and "没有找到续播文件。"
            or "启用 --write-filename-in-watch-later-config 可选择最近的文件。")
        return
    end
...
local function edit_config_file(filename)
    if not mp.get_property_bool("config") then
        show_warning("使用 --no-config 时不支持编辑配置文件。")
        return
    end
...
mp.add_key_binding(nil, "menu", function ()
    ...
    local menu = {
        {"字幕", "script-binding select/select-sid", sub_track_count > 0},
        {"辅助字幕", "script-binding select/select-secondary-sid", sub_track_count > 1},
        {"字幕行", "script-binding select/select-subtitle-line", text_sub_selected},
        {"音轨", "script-binding select/select-aid", audio_track_count > 1},
        {"视频轨", "script-binding select/select-vid", video_track_count > 1},
        {"播放列表", "script-binding select/select-playlist",
         mp.get_property_native("playlist-count") > 1},
        {"章节", "script-binding select/select-chapter", mp.get_property("chapter")},
        {is_disc and "标题" or "版本", "script-binding select/select-edition",
         mp.get_property_native("edition-list/count", 0) > 1},
        {"音频设备", "script-binding select/select-audio-device", audio_track_count > 0},
        {"按键绑定", "script-binding select/select-binding", true},
        {"历史", "script-binding select/select-watch-history", true},
        {"续播", "script-binding select/select-watch-later", true},
        {"播放统计", "script-binding stats/display-page-1-toggle", true},
        {"文件信息", "script-binding stats/display-page-5-toggle",
         mp.get_property("filename")},
        {"编辑配置文件", "script-binding select/edit-config-file", true},
        {"编辑按键绑定", "script-binding select/edit-input-conf", true},
        {"帮助", "script-binding stats/display-page-4-toggle", true},
        {"在线文档", "script-binding select/open-docs", true},
    }
